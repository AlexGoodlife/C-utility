...
static void insertion_sort(void* a, size_t data_size, int(*cmp)(const void*,const void*), size_t low, size_t high, size_t initial_sorted, void* tmp)
{
  assert(low <= high);
...
void invert_array(void*a, size_t low, size_t high, size_t data_size, void* tmp){
  // void* temp = malloc(data_size);
  while(low < high){
    swap((cutil_uint8*)a + low * data_size, (cutil_uint8*) a + high*data_size, data_size,tmp);
    low++;
    high--;
  }
  // free(temp);
...
_cutil_run get_run_or_ascend(void* a, size_t low, size_t high,size_t data_size, int(*cmp)(const void*, const void*), void* tmp)
{
  assert(low <= high);
  ...
  if(cmp((cutil_uint8*)a + low * data_size, (cutil_uint8*)a + (low+1)*data_size) < 0){
    ...
    invert_array(a, low, low + result.length-1, data_size,tmp);
  }
  return result;
}
...
_cutil_run run_with_min_size(void* a,size_t n, size_t data_size, int(*cmp)(const void*,const void*), size_t low, size_t high, size_t min_run_size, void* tmp)
{
  assert(low <= high);
  assert(min_run_size > 0);

  _cutil_run result = get_run_or_ascend(a,low,high,data_size,cmp,tmp);

  if(result.length < min_run_size){
    insertion_sort(a,data_size,cmp,low, math_min(min_run_size+ low - 1, high), result.length,tmp);
    result = get_natural_run(a,low,high,data_size,cmp);
  }
...
void merge_t(cutil_uint8* a, cutil_uint8* aux, _cutil_run leftRun, _cutil_run rightRun, size_t data_size, int(*cmp)(const void*, const void*)){
  int low = leftRun.start;
  int high = rightRun.start+rightRun.length-1;

  int left = leftRun.start;
  int right = rightRun.start;
  int mid = leftRun.start + leftRun.length-1;
        
  size_t size = 0;
  size_t i = 0;
  size_t j = 0;
  cutil_uint8* b = a + (right)*data_size;
  size_t m = high-right+1;
  size_t n = mid-low+1;
  a = a + low*data_size;
  while(i < n && j < m){
    if(cmp((a + (data_size*i)), (b + (data_size*j))) <= 0){
      memcpy(aux+(size*data_size), a+(data_size*i),data_size);
      size++;
      i++;
    }
    else{
      memcpy(aux+(size*data_size), b+(data_size*j),data_size);
      size++;
      j++;
    }
  }
  memmove(aux+(size*data_size), (a+i*data_size), (n-i)*data_size);
  size += (n-i);
  memmove(aux+size*data_size, (b+j*data_size), (m-j)*data_size);
  memmove(a, aux, data_size*(high-low+1));
}
...
void merge(cutil_uint8* a,cutil_uint8* buffer,_cutil_run leftRun, _cutil_run rightRun,size_t data_size, int(*cmp)(const void*, const void*))
{
  ...
  merge_t(a,buffer,left,right,data_size,cmp);
...
void sort(void*a, size_t n, size_t data_size, int(*cmp)(const void*, const void*))
{
  void* temp = malloc(data_size); 
  if(n < 64){
    insertion_sort(a,data_size,cmp,0,n-1,0,temp);
    return;
  }

  int k = get_run_size(n);
  cutil_uint8* buffer = (cutil_uint8*)malloc(data_size*(n/1));
  if(!buffer) return;
  ...
  for(size_t i = 0; i < n; i += r.length){
    ...
    r = run_with_min_size(a,n,data_size,cmp,i,n-1,k,temp);
...
#define MEASURE_SIZE 1000000

void merge(cutil_uint8 *a, size_t n, cutil_uint8* b, size_t m, cutil_uint8* temp,size_t data_size,int (*cmp)(const void*,const void*)){
    size_t size = 0;
    size_t i = 0;
    size_t j = 0;
    while(i < n && j < m){
...
void merge_sort_helper(cutil_uint8 *array, size_t array_size, size_t data_size, cutil_uint8* temporary, int (*cmp)(const void*,const void*)){
    if(array_size > 1){
...
void merge_sort(void *array, size_t array_size, size_t data_size, int (*cmp)(const void*,const void*)){
    ...
    merge_sort_helper((cutil_uint8*)array, array_size, data_size, (cutil_uint8*)temporary, cmp);
    free(temporary);
}   

double measure_qsort(int n){

  double total_time = 0;
  int* a =(int*) malloc(sizeof(int)*MEASURE_SIZE);
  for(int i = 0; i < n;i++){
    shuffle_array(a,MEASURE_SIZE);
    auto start = std::chrono::high_resolution_clock::now();
    merge_sort(a, MEASURE_SIZE, sizeof(int),intcmp); 
    
    auto end = std::chrono::high_resolution_clock::now();
    double end_time = std::chrono::duration_cast<std::chrono::milliseconds>(end -start).count();
    total_time += end_time;
  }
  free(a);
  return total_time/n;
...
double measure_tsort(int n){
  ...
  for(int i = 0; i < n;i++){
    shuffle_array(a,MEASURE_SIZE);
    auto start = std::chrono::high_resolution_clock::now();
    sort(a, MEASURE_SIZE, sizeof(int),intcmp); 
...
double measure_qsort_obj(int n){

  double total_time = 0;
  person_t** a =(person_t**) malloc(sizeof(person_t*)*MEASURE_SIZE);
  for(int i = 0; i < n;i++){
    shuffle_array_people(a, MEASURE_SIZE);
    auto start = std::chrono::high_resolution_clock::now();
    qsort(a, MEASURE_SIZE, sizeof(person_t*),personcmp); 
...
double measure_tsort_obj(int n){
  ...
  for(int i = 0; i < n;i++){
    shuffle_array_people(a,MEASURE_SIZE);
    auto start = std::chrono::high_resolution_clock::now();
    sort(a, MEASURE_SIZE, sizeof(person_t*),personcmp); 
...
int main(){
  ...
  printf("qsort: %lf ms, tsort: %lf ms\n", measure_qsort(20), measure_tsort(20));
  printf("ALL GOOD\n");
  return 0;
}
